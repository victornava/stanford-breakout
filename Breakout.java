 ...
 * Name: Victor Nava
 * Date: 15-Nov-2008
 * this my implementation of the Breakout Game
 */
...
/** Dimensions of the paddle */
	...
	private static final int PADDLE_Y_OFFSET = 40;	//Offset of the paddle up from the bottom
	private static final int PADDLE_Y = FRAME_TOP + HEIGHT - PADDLE_Y_OFFSET;	//y coordinate of paddle
...
/** Brick constants */
	...
	private static final int BRICK_Y_OFFSET = FRAME_TOP + ((BRICK_HEIGHT + BRICK_SEP) * 6); //Offset of the top brick row from the top
	
	private static final int BRICK_WIDTH =
		  (WIDTH - (NBRICKS_PER_ROW - 1) * BRICK_SEP) / NBRICKS_PER_ROW;	//width of bricks	
	
	private static final Color[] BRICK_COLOR =
	{Color.red, Color.orange, Color.yellow, Color.green, Color.cyan};		//colors of bricks
...
/** Instance variables */
	...
	private static double xVelBefore = xVel;	//to keep trak of direction of ball before a bounce
	private static double yVelBefore = yVel;	//to keep trak of direction of ball before a bounce
	private static int bricksLeft;				//number of remaining bricks
	private static int paddleContacts;			//number times the ball has hit the paddle
	private static String currentCollider;		//name of colliding object
	private static boolean JustBoncedInPaddle;	//to prevent the ball bouncing twice on paddle
...
/** instance objects */	
	...
	private GOval[] ballIndicator = {new GOval(1,1), new GOval(1,1), new GOval(1,1)}; 
	
	AudioClip audioClip = MediaTools.loadAudioClip("drip.au");	//bounce audioclip
...
/** Runs the Breakout program. */
					...
					if (collider == null){
						...
						if (colliderWidth >= WIDTH){
							...
							//if collider is paddle...
								...
								if (paddleContacts % 7 == 0 && (Math.abs(yVel) * 1.1) < maxVel){
									yVel+= yVel*0.1;	//increase y Velocity by 10%
								}
								//updateColliderLabel();	//debug only
								//waitForClick();		//debug only
							}
	...
	private void addPaddle(){
		...
		paddle = new GRect( FRAME_LEFT + (WIDTH / 2) - (PADDLE_WIDTH / 2), PADDLE_Y, PADDLE_WIDTH, PADDLE_HEIGHT);
	...
	private GObject getCollidingObject(int ballSection){
		...
		switch(ballSection){
			...
			//ball North
			case 1: return getElementAt(ballLeft + BALL_RADIUS + 2, ballTop);
			
			//ball South 
			case 2: return getElementAt(ballLeft + BALL_RADIUS + 2, ballBottom);
			
			//ball East
			case 3: return getElementAt(ballRight, ballTop + BALL_RADIUS + 2);
			
			//ball West
			case 4:	return getElementAt(ballLeft, ballTop + BALL_RADIUS + 2);
	...
	private void addBallIndicators(){
		for (int i = 0; i < 3; i++){
			...
			remove(ball);
			ballIndicator[i].setLocation (FRAME_RIGHT + BALL_RADIUS, FRAME_TOP + (i * 3 *BALL_RADIUS));
			add(ballIndicator[i]);
		}
	}
	...
	public void mouseMoved(MouseEvent e) { 
		...
		//mousePosition.setLabel(+e.getX()+ "," +e.getY()); //debug only	
	}
}